...
def _prescribe_pid(
    ...
    """
    Suggest P and D gain changes per axis from step-response metrics.

    Consumes the ``Category.PID`` findings produced by the step-response
    diagnostic rule.  Only acts on WARNING-level findings (clearly
    suboptimal responses).
    """
    changes = []
    for f in findings:
        if f.category != Category.PID or f.severity != Severity.WARNING:
            continue
        axis = f.data.get("axis")
        if axis is None:
            continue

        overshoot = float(f.data.get("overshoot_pct", 0.0) or 0.0)
        rise_time = f.data.get("rise_time_s")
        axis_name = f.data.get("axis_name", axis)

        p_key = f"p_{axis}"
        d_key = f"d_{axis}"
        p_cur = _int_or(_get_current(cli, p_key), 50)
        d_cur = _int_or(_get_current(cli, d_key), 30)
        ...
        if overshoot > 20:
            ...
            reason = (
                f"Overshoot {overshoot:.0f}% on {axis_name} — reduce P and "
                f"increase D to damp the response."
            )
        elif overshoot > 12:
            p_new = int(p_cur * 0.95)
            d_new = int(d_cur * 1.05)
            reason = (
                f"Overshoot {overshoot:.0f}% on {axis_name} — slightly reduce "
                f"P and increase D."
            )
        elif rise_time is not None and rise_time > 0.2:
            p_new = int(p_cur * 1.10)
            reason = (
                f"Slow rise time ({rise_time * 1000:.0f} ms) on {axis_name} "
                f"— increase P."
            )
        else:
            continue

        p_new = _clamp(p_new, p_key, schema)
        d_new = _clamp(d_new, d_key, schema)

        if p_new != p_cur:
            changes.append(PrescribedChange(
                setting=p_key, old_value=str(p_cur), new_value=str(p_new),
                reason=reason, finding_title=f.title,
            ))
        if d_new != d_cur:
            changes.append(PrescribedChange(
                setting=d_key, old_value=str(d_cur), new_value=str(d_new),
                reason=reason, finding_title=f.title,
            ))
